...
#             flag: True 表示 self.block.outputQty 的值不等于一. Defaults to False.
#
#         Returns:
...
s1 = [
    {"count": 1, "outputQty": 100, "金锭": 400, "下界合金碎片": 400},
    {"count": 9, "outputQty": 99, "下届合金块": 11},
]
s2 = {
    "金锭": {"outputQty": 1, "stacks": 6, "amount": 16},
    "下界合金碎片": {"outputQty": 1, "stacks": 6, "amount": 16},
}
s3 = {"下届合金块": {"outputQty": 9, "stacks": 0, "amount": 11}}
...
def getChange1(source: list):
    ...
    for recipe in source:
        ...
        for index, num in enumerate(nums[2:]):
            print(Decimal(100 - count * num))

    return


@decorator
def getChange2(source: dict):
